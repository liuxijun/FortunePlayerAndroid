package com.fortune.mobile.media;
...
public class MediaPlayer {
     ...
     *
     * @see com.fortune.mobile.media.MediaPlayer.OnInfoListener
     */
...
/*
        ...
        if (preferHWDecoder) {
            ...
            try {
                ...
                Log.e(TAG,"unloadOMX failed %s", e.toString());
            }
    ...
    static {
        ...
        try {
            ...
            if (!vvo_loaded) {
                ...
                Log.d(TAG,"FALLBACK TO VVO JNI " + vvo_loaded);
            }
...
*/
    ...
    private static boolean loadVAO_native(String vaoPath){
        return false;
    }

    private static boolean loadVVO_native(String vvoPath){
        return false;
    }

    private static boolean loadOMX_native(String omxPath){
        return false;
    }

    private static void unloadOMX_native(){
    }
    ...
    public void setDataSource(Context context, Uri uri, Map<String, String> headers) throws IOException, IllegalArgumentException, SecurityException, IllegalStateException {
        ...
        if (scheme == null || scheme.equals("file")) {
            setDataSource(getPath(uri.toString()));
            return;
        }
     ...
     * @throws IllegalStateException if it is called in an invalid state
     */
    private void _setDataSource(String path, String[] keys, String[] values) throws IOException, IllegalArgumentException, IllegalStateException{
     ...
     * @param cacheDir e.g. getCacheDir().toString()
     */
    private void _setDataSegmentsSource(String[] segments, String cacheDir){
    ...
    public void start() throws IllegalStateException {
        stayAwake(true);
        if (mInBuffering) {
            //Log.i(TAG,"MiuiVideo: now is in buffering, and will start after buffering");
            mNeedResume = true;
        } else {
            //Log.i(TAG,"MiuiVideo: start player");
            _start();
        }
    }
    ...
    public void pause() throws IllegalStateException {
        stayAwake(false);
        mNeedResume = false;
        //Log.i(TAG,"MiuiVideo: pause player");
        _pause();
    }
    ...
    public void setIsPlaying(boolean isPlayerPlaying){
    ...
    private boolean native_getTrackInfo(SparseArray<byte[]> trackSparse){
        return false;
    }

    private boolean native_getMetadata(Map<byte[], byte[]> meta){
        return false;
    }
     ...
     * @see com.fortune.mobile.media.MediaPlayer#getTrackInfo
     */
    ...
    public  void audioInitedOk(long bufSize){
    ...
    /**
     * @return the percent
     * @see com.fortune.mobile.media.MediaPlayer.OnBufferingUpdateListener
     */
...
//	  Log.e(TAG,"  ffff mediaplayer audiotrackinit start .  sampleRateInHz:=" + sampleRateInHz + " channels:=" + channels );
        audioTrackRelease();
    ...
    private class EventHandler extends Handler {
        ...
        private void onInfo(Message msg) {
            switch (msg.arg1) {
                ...
                {
                    //Log.i(TAG,"MiuiVideo: receive buffer start, isplaying " + isPlaying());
                    mInBuffering = true;
                    if (isPlaying()) {
                        //Log.i(TAG,"MiuiVideo: pause player for buffer start");
                        _pause();
                        mNeedResume = true;
                    }
                    break;
                ...
                {
                    //Log.i(TAG,"MiuiVideo: receive buffer end, needResume = " + mNeedResume);
                    mInBuffering = false;
                    if (mNeedResume) {
                        //Log.i(TAG,"MiuiVideo: start player after buffer");
                        _start();
                        mNeedResume = false;
                    }
                    break;
        ...
        private void onBufferingUpdate(Message msg) {
            ...
            if (percent >= 100 && mInBuffering) {
                //Log.i(TAG,"MiuiVideo: receive buffer 100, needResume = " + mNeedResume);
                mInBuffering = false;
                if (mNeedResume) {
                    //Log.i(TAG,"MiuiVideo: start player after buffer 100");
                    _start();
                    mNeedResume = false;
                }
                if (mOnInfoListener != null) {
                    //Log.i(TAG,"MiuiVideo: add one buffer end event");
...
//        //Log.i(TAG,"MiuiVideo: get message after player released, msg type: " + msg.what);
                return;
            }
            switch (msg.what) {
                case MEDIA_PREPARED:
                ...
                case MEDIA_TIMED_TEXT:
                    ...
                    if (mData.getInt(MEDIA_SUBTITLE_TYPE) == SUBTITLE_TEXT) {
                        Log.i(TAG,"Subtitle : "+mData.getString(MEDIA_SUBTITLE_STRING));
